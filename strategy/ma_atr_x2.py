...
from strategy.common.utils import(
    round_at, lot_round_at, round_it, upated_after_closed)
...
from prototype.v4 import get_feat, get_signal, get_prices
...
def main(args):
    ...
    pm = PositionManager(args.stgname)
    position = pm.load()
    upated_after_closed(args, cli, position)
    ...
    ma_idx = gdf.columns.get_loc('MA')
    atr_idx = gdf.columns.get_loc('ATR')
    cond_l, cond_s, atr = get_signal(
        gdf.values, price, args.k, args.clen, atr_idx, ma_idx)
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument('--clen', type=int, default=2, help='for MA window in this case')
