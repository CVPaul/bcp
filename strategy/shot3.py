...
class KLineRobinQueue:
    ...
    def init(self, cli, symbol):
        assert self.interval >= 60000
        data = cli.klines(
            symbol, f'{self.interval//60000}m', limit=self.maxlen)
        for i in range(len(data)):
            self.queue[i] = [data[i][0]] + [float(x) for x in data[i][1:5]]
        self.pos = i
...
last_short_close_time = 0

def on_message(self, message):
    global krq, args, lut
    global trade_cum_period, trade_cum_count_per_1m
    global last_long_close_time, last_short_close_time
    ...
    if etype == 'bookTicker':
        ...
        if args.side == 1 and bid_p <= args.hpp * (1 - S):
            ...
            trade_cnt += 1
            last_long_close_time = message["E"]
            logging.info(
        ...
        if args.side == -1 and ask_p >= args.lpp * (1 + S):
            ...
            args.stopi = 1 # notation, this is start with 1
            trade_cnt += 1
            last_short_close_time = message["E"]
            logging.info(
        ...
        hpp, lpp = krq.gap()
        cond = [
            message["E"] >= last_long_close_time + args.silent_time,
        ...
        cond = [
            message["E"] >= last_short_close_time + args.silent_time,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--silent-time', type=int, default=300000) # 5 min
    ...
    args.silent_time = min(args.silent_time, args.length * args.interval)
    ...
    if args.interval >= 60000:
        krq.init(client, args.symbol)
    logging.info(
