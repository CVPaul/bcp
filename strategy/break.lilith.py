...
def on_tick(cli, bid_p, ask_p):
    actions = []
    no = args.kline[1]
    if args.kline[0] != args.last_kline[0]:
        # update upp/dnn
        args.upp.update(kline[0],args.last_kline[2])
        args.dnn.update(kline[0],args.last_kline[3])
    ...
    if args.mp != 0:
        ...
        if args.mp > 0 and nh >= spp: # 止盈
            args.mp = 0
            actions.append({
                'side':'SELL', 'price':spp, 'newClientId':'win',
                'type':'LIMIT', 'timeInForce':'GTC'})
        # short
        spp = args.enpp + args.s1 * args.atr.value
        spp = round(spp, ROUND_AT[args.symbol])
        if args.mp < 0 and nh >= spp: # 止损
            ...
            actions.append({
                ...
                'type':'LIMIT', 'timeInForce':'GTC'})
            actions.append(('BUY', spp, 'loss', 'LIMIT', 'GTC'))
        spp = args.enpp - args.s2 * args.atr.value
        spp = round(spp, ROUND_AT[args.symbol])
        if args.mp < 0 and nl <= spp: # 止盈
            args.mp = 0
            actions.append({
                'side':'BUY', 'price':spp, 'newClientId':'win',
                'type':'LIMIT', 'timeInForce':'GTC'})
    return actions


def trade(cli, actions, T):
...
def on_message(self, message):
    ...
    if etype == 'bookTicker':
        ...
        if start_t != args.kline[0]:
            ...
            args.kline[4] = mprice
        actions = on_tick(client, args.kline)
        trade(client, actions, message['E'])
        ...
        if 'lilith' in args.stgname:
            ...
            while True:
                try:
                    ...
                    on_tick(client, kline)
                    time.sleep(0.2) # 200ms
                except Exception as e:
                    logging.exception(f"on_tick failed with error:{e}")
                # client.renew_listen_key(listen_key)
        else:
            raise RuntimeError(
